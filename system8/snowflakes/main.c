...
int snowflakes(double x0, double y0);
...
void demo(double x, double y)
{
    int c = snowflakes(x, y);
    printf("snowflakes(%f, %f) = %d\n", x, y, c);
}
...
void init_colors(png_byte colors[MAX_COLORS][3])
{
	...
	{
		double x = (double)(c-1) / (double)(MAX_COLORS-1);
		x = pow(x, 0.5); // 0.25);
		if(x < 0.5) 
		{
